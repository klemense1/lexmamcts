...
public:
  ...
  std::vector<SE> GetHeuristicValues(const std::shared_ptr<StageNode<S, SE, SO, H>> &node) {
    ...
    if (node->GetState()->IsTerminal()) {
      ...
      for (AgentIdx ai = 0; ai < num_agents; ++ai) {
        statistics.emplace_back(node->GetState()->GetNumActions(ai), mcts_parameters_);
        statistics.at(ai).SetHeuristicEstimate(Reward::Zero(mcts_parameters_.REWARD_VEC_SIZE));
      }
      return statistics;
    }
    ...
    while ((!state->IsTerminal()) && (num_iterations < mcts_parameters_.random_heuristic.MAX_NUMBER_OF_ITERATIONS)
      ...
      auto new_state = state->Execute(RandomJointAction(state, num_agents), step_rewards);
  ...
  template<class S>
  JointAction RandomJointAction(const std::shared_ptr<S> &state, AgentIdx num_agents) {
    JointAction ja(num_agents);
    for(size_t i = 0; i < num_agents; ++i) {
      std::uniform_int_distribution<ActionIdx> random_action_selection(0,state->GetNumActions(i)-1);
