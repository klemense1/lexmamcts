...
#ifndef MCTS_STATISTICS_PARETO_UCT_STATISTIC_H_
#define MCTS_STATISTICS_PARETO_UCT_STATISTIC_H_
...
class ParetoUCTStatistic : public UctStatistic<ParetoUCTStatistic> {
 public:
  ParetoUCTStatistic(ActionIdx num_actions,
      ...
      : UctStatistic<ParetoUCTStatistic>(num_actions, mcts_parameters) {}
  ...
  void calculate_modified_ucb_values(
      const ActionUCBMap &ucb_statistics,
      std::vector<Eigen::VectorXf> &values) const {
    values.resize(ucb_statistics.size());

    for (size_t idx = 0; idx < ucb_statistics.size(); ++idx) {
      Eigen::VectorXf action_value_normalized =
          (ucb_statistics.at(idx).action_value_ -
           this->mcts_parameters_.uct_statistic.LOWER_BOUND)
              .cwiseQuotient(this->mcts_parameters_.uct_statistic.UPPER_BOUND -
                             this->mcts_parameters_.uct_statistic.LOWER_BOUND);
      ...
      values[idx] = action_value_normalized.array() +
                    sqrt((4 * log(total_node_visits_) +
                          log(this->mcts_parameters_.REWARD_VEC_SIZE)) /
                         (2 * ucb_statistics.at(idx).action_count_));
    }
  }
};
...
#endif  // MCTS_STATISTICS_PARETO_UCT_STATISTIC_H_
