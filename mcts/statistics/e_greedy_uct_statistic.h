...
#ifndef MCTS_STATISTICS_E_GREEDY_UCT_STATISTIC_H_
#define MCTS_STATISTICS_E_GREEDY_UCT_STATISTIC_H_
...
class EGreedyUCTStatistic : public UctStatistic<EGreedyUCTStatistic> {
 public:
  EGreedyUCTStatistic(ActionIdx num_actions,
                      MctsParameters const &mcts_parameters)
      : UctStatistic<EGreedyUCTStatistic>(num_actions, mcts_parameters) {}
  ...
  ActionIdx choose_next_action(const S &state,
    ...
    ActionIdx selected_action =
        UctStatistic<EGreedyUCTStatistic>::choose_next_action(
            state, unexpanded_actions);
    if (fully_expanded) {
      std::uniform_real_distribution<float> uniform_norm(0, 1);
      if (uniform_norm(random_generator_) >=
        ...
        return random_action == selected_action ? random_action + 1
                                                : random_action;
      }
    } else {
      return selected_action;
...
#endif  // MCTS_STATISTICS_E_GREEDY_UCT_STATISTIC_H_
