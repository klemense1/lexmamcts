...
void define_rule_monitor(py::module m) {
  py::class_<RuleMonitor, std::shared_ptr<RuleMonitor>>(m, "RuleMonitor")
      ...
      .def(py::pickle(
          [](const RuleMonitor &b) {
            return py::make_tuple(b.get_str_formula(), b.get_weight(), b.get_priority(), b.get_init_belief(),
                                  b.get_final_reward1());
          },
          [](py::tuple t) {
            if (t.size() != 5) throw std::runtime_error("Invalid RuleMonitor evaluator state!");
            return RuleMonitor::make_rule(t[0].cast<std::string>(), t[1].cast<float>(), t[2].cast<RulePriority>(),
                                          t[3].cast<float>(), t[4].cast<float>());
          }));
