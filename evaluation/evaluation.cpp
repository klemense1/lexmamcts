...
QValWriter::QValWriter(const Eigen::VectorXf& thres, const std::string& filename) : thres_(thres), timestamp_(0) {
...
QValWriter::~QValWriter() {
  ofstream_.close();
}

int QValWriter::FindLexMax(const std::map<unsigned long, Eigen::VectorXf>& action_val_map, unsigned long best_action) {
