...
#ifndef TEST_CROSSING_TEST_FACTORIES_PARETO_CONVERGENCE_TEST_ENV_FACTORY_H_
#define TEST_CROSSING_TEST_FACTORIES_PARETO_CONVERGENCE_TEST_ENV_FACTORY_H_

#include "test/crossing_test/factories/test_env_factory.h"

#include <utility>
template <class Stat>
class ParetoConvergenceTestEnvFactory : public ITestEnvFactory {
 public:
  explicit ParetoConvergenceTestEnvFactory(ObjectiveVec thres = make_default_mcts_parameters().thres_uct_statistic_.THRESHOLD,
  ...
  std::shared_ptr<BaseTestEnv> make_test_env() override {
    ...
    agent_states[0].x_pos = 4;
    agent_states[1].x_pos = 6;
...
#endif  // TEST_CROSSING_TEST_FACTORIES_PARETO_CONVERGENCE_TEST_ENV_FACTORY_H_
